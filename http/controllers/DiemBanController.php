<?php namespace Thaiminh\Diemban\Http\Controllers;
...
class DiemBanController extends Controller
{
    ...
    protected $show_district;
    protected $local_region_map;
    protected $provinces;
    protected $regions;
    ...
    */
    ...
    public function __construct()
    {
        ...
        $this->provinces = array_map(function ($province) {
            if(in_array($province['id'], $this->show_district)){
                $province['districts'] = $this->get_district_by_province_id($province['id']);
            }
            return $province;
        }, $this->provinces());
     ...
     * @return Response
     */
    public function pageDiemBan(Request $request)
    {
        $theme = Theme::getActiveTheme();
        $heading = Settings::get('heading_page');
        ...
        return view('thaiminh.diemban::pageDiemBan', compact('heading', 'provinces', 'regions', 'show_district', 'base_url', 'local_region_map'));
    }

    public function pageDiemBanDetail(Request $request, $location)
    {
        $heading = Settings::get('heading_page_detail');
        $base_url = config('app.url');
        $f_location = $this->get_location_by_slug($location);
        $f_province = isset($f_location) && !empty($f_location['data']['province']) ? $f_location['data']['province'] : '';
        $f_district = isset($f_location) && !empty($f_location['data']['province']) && !empty($f_location['data']['district']) ? $f_location['data']['district'] : '';
        $show_district = $this->show_district;
        $provinces = $this->provinces;
        $regions = $this->regions;
        $local_region_map = $this->local_region_map;

        return view('thaiminh.diemban::pageDiemBanDetail', compact('f_district', 'f_province', 'heading', 'provinces', 'regions', 'show_district', 'base_url', 'local_region_map'));
